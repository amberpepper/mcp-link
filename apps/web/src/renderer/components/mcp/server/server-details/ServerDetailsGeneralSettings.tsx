...
import { MCPServer } from "@mcp_link/shared";
import { useTranslation } from "react-i18next";
import { Info, FileText, Plus, Trash, Terminal } from "lucide-react";
import { Button } from "@mcp_link/ui";
import { Input } from "@mcp_link/ui";
import { Label } from "@mcp_link/ui";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <Label className="text-base font-medium flex items-center gap-1.5">
          <Terminal className="h-4 w-4 text-muted-foreground" />
          {t("serverDetails.timeouts")}
        </Label>
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor="startup-timeout" className="text-sm">
              {t("serverDetails.startupTimeout")}
            </Label>
            <Input
              id="startup-timeout"
              type="number"
              min={1}
              value={editedStartupTimeoutSec}
              onChange={(event) =>
                setEditedStartupTimeoutSec(clampTimeout(event.target.value, 45))
              }
            />
            <p className="text-xs text-muted-foreground">
              {t("serverDetails.startupTimeoutDescription")}
            </p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="capability-timeout" className="text-sm">
              {t("serverDetails.capabilityTimeout")}
            </Label>
            <Input
              id="capability-timeout"
              type="number"
              min={1}
              value={editedCapabilityTimeoutSec}
              onChange={(event) =>
                setEditedCapabilityTimeoutSec(
                  clampTimeout(event.target.value, 15),
                )
              }
            />
            <p className="text-xs text-muted-foreground">
              {t("serverDetails.capabilityTimeoutDescription")}
            </p>
          </div>
        </div>
      </div>
...
function clampTimeout(value: string, fallback: number) {
