# @mcp_link/tailwind-config

Shared Tailwind CSS configuration for the MCP Link monorepo.
...
@import "@mcp_link/tailwind-config/theme.css";
...
```js
module.exports = {
  presets: [require("@mcp_link/tailwind-config/tailwind.config.js")],
  content: [
...
```js
const { colors, radius } = require("@mcp_link/tailwind-config");
...
```js
const { generateThemeCSS } = require("@mcp_link/tailwind-config");
