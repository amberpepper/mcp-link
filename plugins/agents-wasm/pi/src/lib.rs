...
impl AgentPlugin for Pi {
    fn handle(method: &str, params: &Value) -> Result<Value, String> {
        match method {
            ...
            _ => Err(format!("Unsupported Pi method: {method}")),
        }
    }
}
...
fn parse_session_content(file: &str, content: &str, full: bool, base_offset: u64) -> Value {
    ...
    let title = title
        ...
        .unwrap_or_else(|| "Untitled Pi session".to_string());
    json!({
        "id": format!("pi:{native_id}"),
        "agentId": "pi",
        "nativeId": native_id,
...
fn message_value(file: &str, offset: u64, role: &str, text: &str, model: Option<&str>) -> Value {
    json!({
        "id": format!("pi-message:{file}:{offset}"),
        "role": role,
        "kind": "text",
        "text": text,
        "model": model,
...
fn required_native_id(params: &Value) -> Result<&str, String> {
    params
        .get("nativeId")
        .and_then(Value::as_str)
        .ok_or_else(|| "Pi nativeId is required".to_string())
}
...
fn find_session(native_id: &str) -> Result<String, String> {
    session_files()?
        ...
        .ok_or_else(|| format!("Pi session not found: {native_id}"))
}
